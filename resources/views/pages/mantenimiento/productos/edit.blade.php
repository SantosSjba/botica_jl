...
@php
    ...
    $p = $producto;
    $fechaVenc = $p->lote && $p->lote->fecha_vencimiento ? $p->lote->fecha_vencimiento->format('Y-m-d') : '';
@endphp
        ...
        <form action="{{ route('mantenimiento.productos.update', $producto) }}" method="post" x-data="{ loading: false }" @submit="loading = true" class="space-y-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" id="codigo" name="codigo" value="{{ old('codigo', $p->codigo) }}" placeholder="Código de barra"
                ...
                <div>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="idlote" name="idlote" required class="{{ $selectClass }} @error('idlote'){{ $errorClass }}@enderror">
                            @foreach($lotes as $l)
                                <option value="{{ $l->idlote }}" data-fecha="{{ $l->fecha_vencimiento ? $l->fecha_vencimiento->format('Y-m-d') : '' }}" {{ old('idlote', $p->idlote) == $l->idlote ? 'selected' : '' }}>{{ $l->numero }} @if($l->fecha_vencimiento)(Vence: {{ $l->fecha_vencimiento->format('d/m/Y') }})@endif</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="date" id="fecha_vencimiento" name="fecha_vencimiento" value="{{ old('fecha_vencimiento', $fechaVenc) }}"
                ...
                <div>
                    ...
                    <input type="text" id="descripcion" name="descripcion" value="{{ old('descripcion', $p->descripcion) }}" required placeholder="Descripción del producto"
                ...
                <div>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="tipo" name="tipo" required class="{{ $selectClass }} @error('tipo'){{ $errorClass }}@enderror">
                            <option value="Generico" {{ old('tipo', $p->tipo) === 'Generico' ? 'selected' : '' }}>Genérico</option>
                            <option value="No Generico" {{ old('tipo', $p->tipo) === 'No Generico' ? 'selected' : '' }}>No genérico</option>
                            <option value="No Aplica" {{ old('tipo', $p->tipo) === 'No Aplica' ? 'selected' : '' }}>No aplica</option>
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                        </span>
                ...
                <div>
                    ...
                    <input type="number" id="stock" name="stock" value="{{ old('stock', $p->stock) }}" required min="0"
                ...
                <div>
                    ...
                    <input type="number" id="stockminimo" name="stockminimo" value="{{ old('stockminimo', $p->stockminimo) }}" required min="0"
                ...
                <div>
                    ...
                    <input type="number" id="precio_compra" name="precio_compra" value="{{ old('precio_compra', $p->precio_compra) }}" required min="0" step="0.01"
                ...
                <div>
                    ...
                    <input type="number" id="precio_venta" name="precio_venta" value="{{ old('precio_venta', $p->precio_venta) }}" required min="0" step="0.01"
                ...
                <div>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="ventasujeta" name="ventasujeta" required class="{{ $selectClass }} @error('ventasujeta'){{ $errorClass }}@enderror">
                            <option value="sin receta medica" {{ old('ventasujeta', $p->ventasujeta) === 'sin receta medica' ? 'selected' : '' }}>Sin receta médica</option>
                            <option value="Con receta medica" {{ old('ventasujeta', $p->ventasujeta) === 'Con receta medica' ? 'selected' : '' }}>Con receta médica</option>
                            <option value="No aplica" {{ old('ventasujeta', $p->ventasujeta) === 'No aplica' ? 'selected' : '' }}>No aplica</option>
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                        </span>
                ...
                <div>
                    ...
                    <input type="date" id="fecha_registro" name="fecha_registro" value="{{ old('fecha_registro', $p->fecha_registro ? $p->fecha_registro->format('Y-m-d') : date('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="reg_sanitario" name="reg_sanitario" value="{{ old('reg_sanitario', $p->reg_sanitario) }}" placeholder="Registro sanitario"
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="idcategoria" name="idcategoria" required class="{{ $selectClass }} @error('idcategoria'){{ $errorClass }}@enderror">
                            @foreach($categorias as $c)
                                <option value="{{ $c->idcategoria }}" {{ old('idcategoria', $p->idcategoria) == $c->idcategoria ? 'selected' : '' }}>{{ $c->forma_farmaceutica }}</option>
                            @endforeach
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="idpresentacion" name="idpresentacion" required class="{{ $selectClass }} @error('idpresentacion'){{ $errorClass }}@enderror">
                            @foreach($presentaciones as $pr)
                                <option value="{{ $pr->idpresentacion }}" {{ old('idpresentacion', $p->idpresentacion) == $pr->idpresentacion ? 'selected' : '' }}>{{ $pr->presentacion }}</option>
                            @endforeach
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="idcliente" name="idcliente" class="{{ $selectClass }} @error('idcliente'){{ $errorClass }}@enderror">
                            ...
                            @foreach($laboratorios as $lab)
                                <option value="{{ $lab->idcliente }}" {{ old('idcliente', $p->idcliente) == $lab->idcliente ? 'selected' : '' }}>{{ $lab->nombres }}</option>
                            @endforeach
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="idsintoma" name="idsintoma" required class="{{ $selectClass }} @error('idsintoma'){{ $errorClass }}@enderror">
                            @foreach($sintomas as $s)
                                <option value="{{ $s->idsintoma }}" {{ old('idsintoma', $p->idsintoma) == $s->idsintoma ? 'selected' : '' }}>{{ $s->sintoma }}</option>
                            @endforeach
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="idtipoaf" name="idtipoaf" required class="{{ $selectClass }} @error('idtipoaf'){{ $errorClass }}@enderror">
                            @foreach($tiposAfectacion as $ta)
                                <option value="{{ $ta->idtipoa }}" {{ old('idtipoaf', $p->idtipoaf) == $ta->idtipoa ? 'selected' : '' }}>{{ $ta->descripcion }}</option>
                            @endforeach
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                ...
                <div>
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="tipo_precio" name="tipo_precio" required class="{{ $selectClass }} @error('tipo_precio'){{ $errorClass }}@enderror">
                            <option value="01" {{ old('tipo_precio', $p->tipo_precio) === '01' ? 'selected' : '' }}>Trabaja con IGV</option>
                            <option value="02" {{ old('tipo_precio', $p->tipo_precio) === '02' ? 'selected' : '' }}>Valor referencial unitario</option>
                        </select>
                        <span class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4.79175 7.396L10.0001 12.6043L15.2084 7.396" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>
                        </span>
