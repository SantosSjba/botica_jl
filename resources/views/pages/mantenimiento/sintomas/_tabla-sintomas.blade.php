...
@endphp
<div class="sintomas-tabla-container">
                ...
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        <th class="px-5 py-3 text-left sm:px-6">
                            <a href="{{ route('mantenimiento.sintomas.index', array_merge($baseQuery, ['sort' => 'sintoma', 'direction' => $sortDir('sintoma')])) }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">Síntoma {{ $sortIcon('sintoma') }}</a>
                        </th>
                    ...
                    @forelse($sintomas as $s)
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td class="px-5 py-4 sm:px-6">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $s->sintoma }}</p>
                            </td>
    ...
    @if($sintomas->hasPages())
        <div class="mt-4 flex flex-col items-center justify-between gap-2 border-t border-gray-100 pt-4 dark:border-gray-800 sm:flex-row sm:gap-4">
            <p class="text-theme-sm text-gray-500 dark:text-gray-400">
                Mostrando {{ $sintomas->firstItem() }} a {{ $sintomas->lastItem() }} de {{ $sintomas->total() }} resultados
            </p>
            <nav class="flex items-center gap-1" aria-label="Paginación">
                @if ($sintomas->onFirstPage())
                ...
                @else
                    <a href="{{ $sintomas->withQueryString()->previousPageUrl() }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-white/[0.03]">‹</a>
                @endif
                @foreach ($sintomas->getUrlRange(max(1, $sintomas->currentPage() - 2), min($sintomas->lastPage(), $sintomas->currentPage() + 2)) as $page => $url)
                    @if ($page == $sintomas->currentPage())
                    ...
                    @endif
                @endforeach
                @if ($sintomas->hasMorePages())
                    <a href="{{ $sintomas->withQueryString()->nextPageUrl() }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-white/[0.03]">›</a>
                @else
                    <span class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-gray-100 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500">›</span>
                @endif
            </nav>
