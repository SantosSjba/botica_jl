...
<div class="min-w-0 space-y-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="border-b border-gray-200 px-4 py-4 dark:border-gray-800 sm:px-6">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Actualizar Cliente / Laboratorio</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">(*) Campos obligatorios</p>
        </div>
        <form action="{{ route('mantenimiento.clientes.update', $cliente) }}" method="post" x-data="{ loading: false }" @submit="loading = true" class="p-4 sm:p-6">
            @csrf
            @method('PUT')
            @if ($errors->any())
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="id_tipo_docu" name="id_tipo_docu" required
                        ...
                        @foreach($tiposDocumento as $td)
                            <option value="{{ $td->idtipo_docu }}" {{ old('id_tipo_docu', $cliente->id_tipo_docu) == $td->idtipo_docu ? 'selected' : '' }}>{{ $td->descripcion }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input type="text" id="nrodoc" name="nrodoc" value="{{ old('nrodoc', $cliente->nrodoc) }}" required
                ...
                <div>
                    ...
                    <select id="tipo" name="tipo" required
                        ...
                        <option value="laboratorio" {{ old('tipo', $cliente->tipo) === 'laboratorio' ? 'selected' : '' }}>Laboratorio</option>
                    </select>
                </div>
            </div>
            <div class="mt-6 flex flex-wrap gap-3">
                <x-ui.button-loader type="submit" label="Modificar" loading-text="Guardando..." class="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600" />
