...
<div class="min-w-0 space-y-6">
    ...
    <x-common.component-card title="Usuario" desc="Listado de usuarios. Busque, ordene por columnas o agregue nuevos registros.">
        <div class="space-y-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <form id="form-buscar-usuarios" method="get" action="{{ route('mantenimiento.usuarios.index') }}" class="flex flex-wrap items-end gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <a href="{{ route('mantenimiento.usuarios.create') }}">
                    ...
                    <x-ui.button type="button" id="btn-actualizar-usuarios" variant="outline" size="md" class="inline-flex items-center gap-2" title="Actualizar datos">
            ...
            <x-ui.content-loading
                wrapperId="usuarios-tabla-wrapper"
                loadingId="usuarios-tabla-loading"
                contentContainerId="usuarios-tabla-container"
            >
                @include('pages.mantenimiento.usuarios._tabla-usuarios')
            </x-ui.content-loading>
        </div>
...
<script>
(function() {
    var baseUrl = @json(route('mantenimiento.usuarios.index'));
    var wrapper = document.getElementById('usuarios-tabla-wrapper');
    var container = document.getElementById('usuarios-tabla-container');
    var loadingEl = document.getElementById('usuarios-tabla-loading');
    ...
    document.getElementById('btn-actualizar-usuarios').addEventListener('click', function() {
        aplicarBusqueda();
    });

    wrapper.addEventListener('click', function(e) {
        var a = e.target.closest('a[href*="mantenimiento/usuarios"]');
        if (!a) return;
