        ...
        <form action="{{ route('mantenimiento.presentaciones.store') }}" method="post" x-data="{ loading: false }" @submit="loading = true" class="space-y-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" id="presentacion" name="presentacion" value="{{ old('presentacion') }}" required placeholder="Ingrese la presentación" class="{{ $inputClass }} @error('presentacion'){{ $errorClass }}@enderror" />
                </div>
            </div>
