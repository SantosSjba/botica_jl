    ...
    <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="w-full min-w-[700px]">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    <th class="px-5 py-3 text-left sm:px-6">
                        <a href="{{ route('ventas.tickets.index', array_merge($baseQuery, ['sort' => 'idventa', 'direction' => $sortDir('idventa')])) }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">ID {{ $sortIcon('idventa') }}</a>
                    </th>
                    <th class="px-5 py-3 text-left sm:px-6">
                        <a href="{{ route('ventas.tickets.index', array_merge($baseQuery, ['sort' => 'fecha_emision', 'direction' => $sortDir('fecha_emision')])) }}" class="font-medium text-gray-500 text-theme-xs dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">Fecha {{ $sortIcon('fecha_emision') }}</a>
                    </th>
                    <th class="px-5 py-3 text-left sm:px-6">Serie</th>
                    <th class="px-5 py-3 text-left sm:px-6">Correl.</th>
            ...
            <tbody>
                @forelse($ventas as $v)
                    @php
                        $estado = $v->estado ?? $v->feestado ?? 'vigente';
                        $anulado = ($estado === 'anulado');
                        $estadoClass = $anulado ? 'bg-red-50 text-red-700 dark:bg-red-500/15 dark:text-red-500' : 'bg-green-50 text-green-700 dark:bg-green-500/15 dark:text-green-500';
                    @endphp
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <td class="px-5 py-4 sm:px-6">
                            <span class="inline-flex rounded-full px-2 py-0.5 text-theme-xs font-medium {{ $estadoClass }}">{{ $anulado ? 'Anulado' : 'Vigente' }}</span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-5 py-8 text-center text-theme-sm text-gray-500 dark:text-gray-400 sm:px-6">No hay tickets que coincidan con los filtros.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
