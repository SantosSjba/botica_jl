...
<div id="{{ $wrapperId }}" class="relative">
    <div id="{{ $loadingId }}"
        ...
        <div class="flex flex-col items-center gap-2">
            ...
            <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ $loadingText ?? 'Cargando...' }}</span>
        </div>
    </div>
    @php $contentClass = $contentContainerClass ?? ''; @endphp
    <div id="{{ $contentContainerId }}" @if($contentClass !== '') class="{{ $contentClass }}" @endif>
        {{ $slot }}
    </div>
</div>
